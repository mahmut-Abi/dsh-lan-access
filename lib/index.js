...
function configuredHosts(config) {
	...
	if (config.publicHost !== void 0 && config.publicHost !== "") hosts.push(config.publicHost);
	for (const host of hosts) assertTrustedAuthority(host);
	return uniqueHosts(hosts);
}
function displayAuthority(publicHost, fallbackHost, port) {
	const host = publicHost !== void 0 && publicHost !== "" ? publicHost : fallbackHost;
...
function apply(ctx, config = {}) {
	...
	const updateConnectionTrustedHosts = async (enabled) => {
		...
		const nextHosts = enabled ? uniqueHosts([...baseHosts, ...addresses]) : baseHosts;
	...
	const stateOf = async () => {
		...
		const authority = server === void 0 || !enabled ? void 0 : displayAuthority(config.publicHost, primary, server.port);
		return {
			ready: server !== void 0,
			enabled,
			...
			addresses,
			url: authority !== void 0 ? `http://${authority}` : null
		};
	};
	scope.watch((next) => {
...
//#endregion
export { Config, LAN_ACCESS_NAMESPACE, LanAccessSchema, apply, inject, name };
